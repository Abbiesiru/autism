...
#### 2. Calculate Spearman rank correlation of each gene to SORCS2 ####

# Extract SORCS2 expression from matrix
expr_matrix <- GetAssayData(astrocytes_postnatal, slot = "data")
sorcs2_expr <- expr_matrix["SORCS2", ]

# Compute Spearman correlation between SORCS2 and all other genes
correlations <- apply(expr_matrix, 1, function(x) cor(x, sorcs2_expr, method = "spearman"))

# Compute p-values & adj p-values
pvals <- apply(expr_matrix, 1, function(x) cor.test(x, sorcs2_expr, method = "spearman")$p.value)
...
# Save results
write.csv(cor_df, file.path(output_dir, "sorcs2_astrocytes_spearman.csv"), row.names = FALSE)
write.csv(fgsea_res, file.path(output_dir, "sorcs2_gsea_results.csv"), row.names = FALSE)
