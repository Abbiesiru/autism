...
process_rank_and_percent_base <- function(rank_mat, expr_mat, seurat_obj,
                                          cell_types, cell_col, age_col, dataset_label) {

  rank_df <- as.data.frame(as.matrix(t(rank_mat)))  # rows = cells, cols = genes
  expr_df <- as.data.frame(as.matrix(t(expr_mat)))  # same shape
  ...
  for (gene in gene_names) {
    ...
    # Drop NAs again just in case
    df <- df[complete.cases(df), ]
    if (nrow(df) == 0) next  # skip if no data
    
    # Aggregate rank (mean) and expression (% expressing) separately
    agg_rank <- aggregate(df$Rank, by = list(Gene = gene, Cell_Type = df$Cell_Type, Age = df$Age),
                          FUN = mean, na.rm = TRUE)
    
    agg_expr <- aggregate(df$Expr, by = list(Gene = gene, Cell_Type = df$Cell_Type, Age = df$Age),
                          FUN = function(x) mean(x > 0, na.rm = TRUE) * 100)
    
    # Merge both summaries
    agg_df <- merge(agg_rank, agg_expr, by = c("Gene", "Cell_Type", "Age"))
    colnames(agg_df) <- c("Gene", "Cell_Type", "Age", "avg_rank", "percent_expressing")
  ...
  # Combine all gene summaries into one data frame
