...
#### 2. Calculate Spearman rank correlation of each gene to SORCS3 ####

# Extract SORCS3 expression from matrix
expr_matrix <- GetAssayData(astrocytes_postnatal, slot = "data")
SORCS3_expr <- expr_matrix["SORCS3", ]

# Compute Spearman correlation between SORCS3 and all other genes
correlations <- apply(expr_matrix, 1, function(x) cor(x, SORCS3_expr, method = "spearman"))

# Compute p-values & adj p-values
pvals <- apply(expr_matrix, 1, function(x) cor.test(x, SORCS3_expr, method = "spearman")$p.value)
...
# Save results
write.csv(cor_df, file.path(output_dir, "SORCS3_astrocytes_spearman.csv"), row.names = FALSE)
write.csv(fgsea_res, file.path(output_dir, "SORCS3_gsea_results.csv"), row.names = FALSE)
